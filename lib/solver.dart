...
class SolveStrategies {
  ...
  static List<List<int>> _possiblePositionsFrom(Line line, Chunks chunks) {
    // init possiblePositions
    List<List<int>> possiblePositions = <List<int>>[];
    int left = 0;
    int length = line.length() - 1;
    int rr =
        chunks.counts.fold<int>(0, (a, b) => a + b) + chunks.counts.length - 1;
  ...
  SolveStrategies({required this.line, required this.chunks})
      : possiblePositions = _possiblePositionsFrom(line, chunks),
        solvedLine = MutableLine.fromLine(line),
        possibleCellOwners = <Map<int, List<int>>>[],
        existingChunks = PositionedChunks(chunks: []),
        possibleChunkOwners = <Map<int, List<int>>>[] {
  ...
  void updatePossibleOwners() {
    ...
    possibleChunkOwners = List.generate(existingChunks.chunks.length, (i) {
  ...
  void removeChunkExclusives() {
    ...
    List<List<int>> unionPossiblePositions = possibleMappings.fold(
        List<List<int>>.generate(chunks.counts.length, (e) => <int>[]), (m, i) {
  ...
  List<List<int>> copyPossiblePositions(List<List<int>> possiblePositions) {
    return List.generate(possiblePositions.length, (i) {
      var lineClone = <int>[];
  ...
  List<PossibleChunkMappings> findPossibleChunkMappings() {
    var solution = <PossibleChunkMappings>[];
...
class PossibleChunkMappings {
  ...
  PossibleChunkMappings(
      {required this.permutation, required this.possiblePositions});
